...
def main():
    ...
    # global route specification LANE 2
    ...
    header = ['distanceNew', 'timeNew']
    # open the file in the write mode
    #f = open('distances_to_obs.csv', 'w', encoding='UTF8')

    f = open('distances_wo_stl_c2.csv', 'w', encoding='UTF8')
    ...
    while driver.step() != -1:
        ...
        #distance_robot_to_obs = np.linalg.norm(coord_ego - obstacles[0])  # THIS ONE IS THE MOST IMPORTANT
        ...
        #time_sec = (timestep/1000)*counter_step # THIS ONE IS THE MOST IMPORTANT
        #writer.writerow([distance_robot_to_obs,time_sec]) # THIS ONE IS THE MOST IMPORTANT
        # ******************* COMMUNICATION TASKS *********************
        # receive the message
        if receiver.getQueueLength() > 0: 
            ...
            front_car_pos = np.array([buffer[0], buffer[1]]) # np.array([z, x])
            comm_flag = True
            receiver.nextPacket()

            #print("neighboor_car_pos: ", neighboor_car_pos)
            
            distance_robot_to_robot = np.linalg.norm(coord_ego - front_car_pos) 
            time_sec = (timestep/1000)*counter_step
            writer.writerow([distance_robot_to_robot,time_sec])
